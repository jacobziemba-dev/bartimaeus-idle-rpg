...
export class ResourceManager {
  private gold: number = 1000; // Starting gold
  private gems: number = 50;   // Starting gems

  // Idle generation rates (per second)
  private goldPerSecond: number = 0;
  private gemsPerSecond: number = 0;
  ...
  private timeAwaySeconds: number = 0;

  /**
   * Calculate idle generation rate based on current stage
   *
   * @param stageLevel - Current stage the player is on
   */
  updateIdleRates(stageLevel: number): void {
   ...
   * @returns True if purchase successful, false if not enough gold
   */
   ...
   * Try to spend gems
   *
   * @param amount - Gems to spend
   * @returns True if purchase successful, false if not enough gems
   */
   ...
   *
   * @param timeAwayMs - Milliseconds the player was away
   * @param currentStage - Current stage (for calculating rates)
   * @returns Object with gold and gems earned
   */
  calculateOfflineEarnings(timeAwayMs: number, currentStage: number): OfflineEarnings {
   ...
   *
   * @param data - Saved resources data
   */
  fromJSON(data: Partial<ResourcesData>): void {
